...
> #9691/PR#9709 is the same strict-order family. #9806 shares only the *symptom* (lazyBarrel +
...
>   *and its imports*" rule) — current == rewrite == dropped, consistent with default; (b) a
>   non-waived barrel **keeps** the side effect under both models (shadow `dropped=0`) — the
>   barrel earns an `Ordering` obligation and survives. So pruning never drops a *non-waived*
>   side effect; the §7 item is about placement, not correctness loss.
> - **Step-3 touch-points confirmed (code-level):** fixing #9961 is a *coordinated* change —
...
>   `proxy`'s downstream `init_proxy_foo` ref **and** `main`'s `star.foo` resolves by *member
>   resolution* straight to `value` (lib-foo), bypassing `proxy`'s `foo` export — so every one
...
>   snapshot drift. The taint is a *safety floor*, not the final model — worst case it reverts a
...
>   the bug (base, `checkGlobals` dangling) *and* the fix (`[wrapped_module_treeshaking: true]` → `core`
...
>   prove side-effect soundness (non-waived kept, waived dropped — in *both* variants). Full
...
> - **Flag-on sweep (default-on across the *whole* suite, then reverted).** Temporarily flipped
...
>   "string")`, `binding_init_unused_named_import` runs (no `ReferenceError`). #9961's expect_executed:
>   false (its buggy base can't run) is the *only* non-executed case, and its fix is covered
>   structurally (snapshot: `checkGlobals` absent) + by the executed analogues above. This corrects
>   the earlier "step 1 unmet" worry — snapshots are *backed by* execution here.
...
>   The remaining strict-order steps are all *large or policy*: (a) the member-resolution namespace
...
>   The defect is *not* strictExecutionOrder-specific: a module gets `WrapKind::Esm` from **any**
...
>   option is the **legacy-vs-new wrapped-ESM side-effect tree-shaking** switch for *all* wrap
...
>   'used-value'` — so a regression is caught at runtime, not just in the snapshot. (Note: *pure*
...
1. **inclusion / tree-shaking** — `tree_shaking/include_statements.rs`. Honors
   `sideEffects:false` as a *module-pruning* hint.
...
Each bug so far is one *statement shape* the model failed to reconcile, fixed by one more
...
- **strict OFF** (verified): `core` is pruned *as a whole module* — `setupWorker`/`http`
...
3. In `include_statements`, using `setupWorker` includes its **declaring statement**
   (`include_statements.rs:1048-1064`) — and `setupWorker` is a *named import* in `main`,
...
**Root cause:** the init reference is attributed to the *syntactic barrel the import names*
(`core`), not the *canonical owner of the binding actually read* (`setupWorker.js`). A
...
> Note: `sideEffects:false` is a *module-reachability* hint, **not** statement-level purity.
...
- `generate_stage/compute_wrapped_esm_init_metadata.rs` is an **entire pre-finalization
  pass** (`init_is_noop`, `transitive_esm_init_targets`) whose only job is to *re-create*
...
- **`ModuleLive(X)`** — X's body/wrapper must exist in the output. Established by the
  *existing* inclusion machinery, which has many entry points beyond "a binding is read":
...
- **`InitBefore(site, X, reason, await)`** — X must be *initialized* before `site` executes.
  ...
  - **The obligations form an *ordered* relation, not just an edge set.** Order is part of
  ...
  - **BindingInit** — a specific imported binding whose **canonical** lives inside X's wrapped
    ...
    owner)`**, *activated* only when `trigger_symbol ∈ used_symbol_refs`
  ...
  - **Ordering** — X must run its top-level side effects in order. Exists **only** when X
    ...
    pure).
  - **NamespaceInit** (a.k.a. ExportMaterialization) — X's *namespace value* is observed and
    ...
    BindingInit (BindingInit's target is a *symbol*; NamespaceInit's target is the *namespace
    object*).
...
`init_X()` once in *each* importer's prelude and let `__esm`'s `fn = 0` idempotence collapse
the redundant run at runtime. Idempotence is what makes *cross-path redundancy* safe.

Within a single path, placement is a **correctness requirement, not an optimization**:
idempotence guarantees X's body runs at most once, but it does **not** guarantee the *first*
...
> `InitBefore` edges must flow through the *whole* lowering pipeline (statement inclusion →
...
> target is `is_included`. What the model *can* eventually retire is the *reactive
> re-creation* (`transitive_esm_init_targets`), once edges are computed up front.
...
it has a real side effect — replacing the #9709 carve-out. **`#9806` is *not* fixed by this
...
The current output is *known-wrong* for #9961, so "the new model reproduces today's
...
1. **Execution tests first.** Add *node-execution* regression tests (not just snapshots) for
...
2. **Compute candidate obligations in shadow mode.** Add a pass (after
   ...
   equivalence with today; instead diff against current emission and *record the expected
   deltas* (#9961 etc.). No output change yet.
3. **Integrate behind a flag — atomically.** Wire the edges into statement inclusion,
   ...
   Note "keep fallbacks *and* output unchanged" is **not achievable** once edges affect
   inclusion/chunk dependencies — partial integration desyncs the two systems. So either keep
   this step *fully shadow-only* (edges computed, no consumer reads them) **or** put the whole
   ...
   - **Inclusion** — `include_statements.rs`. When a used named import resolves (via
     `canonical_ref_for`, available here) to a canonical owner in a *different* module than the
   ...
   - **Cross-chunk** — `compute_cross_chunk_links.rs:323` must register the *canonical owners'*
...
5. **Retire reconciliation once proven redundant.** Remove `transitive_esm_init_targets` and
   any now-dead guard *only after* the edge invariants demonstrably cover them (the
...
- **Circular deps** — idempotence (`__esm`'s `fn = 0`) makes *redundant re-entry* harmless,
...
- **lazyBarrel — separate, loader-time invariant (cannot be a link-stage edge).** lazyBarrel
  defers records *during module loading* and writes `resolved_module=None`
  ...
  statement that will be kept once the barrel is live — decided from *requested exports* plus
...
*evaluation timing* — through its exported value under a cycle, through TLA, or through
...
metadata. Note the `init_is_noop` pass already drops empty wrappers *post hoc* via
...
- First-dominating-site placement is now a **model requirement** (§2 emission rule), not an
  open question. What remains open is the *implementation*: today's dedup
...
The lazyBarrel/#9806 fixtures gate on the *separate* loader-time change, not the strict-order
rewrite.
