...
/// Collect the canonical names emitted at the chunk's root scope that are captured by every
/// CJS-wrapped module's `__commonJS((exports, module) => { ... })` closure. A real-AST root-scope
/// binding inside the closure whose name matches one of these would shadow the captured value at
/// runtime (issues #9055, #9375, #9630, #9882).
///
/// We track only rolldown-emitted names — iife/umd factory params, `require_xxx` wrapper facades,
/// and wrapped-ESM `xxx_exports` namespace objects — and intentionally exclude import bindings that
/// get rewritten away at codegen time. Wrapper/namespace symbols use their *original* names here:
/// they haven't been renamed yet, and if one ends up renamed in the deconfliction loop, the
/// conflict that triggered it would have been the user-source local — exactly the case we catch.
fn collect_chunk_scope_captured_names(
  ...
  // CJS wrapper facades (e.g. `require_foo`) and wrapped-ESM namespace objects (`var foo_exports =
  // ...`) are rendered at chunk scope and captured by every CJS-wrapped closure in this chunk. The
  // namespace object surfaces via `require()` of the wrapped-ESM module, which becomes
  // `(init_foo(), __toCommonJS(foo_exports))` inside the closure; the `used_symbol_refs` gate
  // mirrors the condition under which its decl is actually rendered (`finalize_normal_module`).
  for module_idx in chunk.modules.iter().copied() {
    ...
    if let Some(module) = link_output.module_table[module_idx].as_normal() {
      let ns_ref = module.namespace_object_ref;
      if link_output.used_symbol_refs.contains(&ns_ref) {
        let canonical_ref = link_output.symbol_db.canonical_ref_for(ns_ref);
        captured.insert(CompactStr::new(canonical_ref.name(&link_output.symbol_db)));
      }
    }
  }
  if matches!(format, OutputFormat::Esm) {
    // A CJS wrapper or wrapped-ESM namespace object whose module lives in *another* chunk is
    // hoisted here as a real root-scope import binding (`import { ... as require_foo } from
    // "./other.js"`) and is likewise captured by every CJS-wrapped closure in this chunk. The
    // in-chunk loop above can't see it because its owner module isn't in `chunk.modules`, so we
    // recover it from the cross-chunk import list. Without this, an author-local of the same name
    // inside a CJS closure shadows the import, emitting the self-referential `var require_foo =
    // require_foo()` (issue #9630) or `__toCommonJS(undefined)` (issue #9882).
    for item in chunk.imports_from_other_chunks.values().flatten() {
      ...
      let is_namespace_object =
        link_output.module_table[canonical_ref.owner].as_normal().is_some_and(|owner| {
          link_output.symbol_db.canonical_ref_for(owner.namespace_object_ref) == canonical_ref
        });
      if is_cjs_wrapper || is_namespace_object {
        captured.insert(CompactStr::new(canonical_ref.name(&link_output.symbol_db)));
      }
    }
  }
